...
void main()
{
    ...
    vec3 norm = fNormalize(fNormal); // 法向向量
    vec3 lightDir = fNormalize(light.position - fFragPos); // 指向光源的向量
    ...
    { // 如果光都没有照射到物体表面，那就不需要计算镜面反射了。
        vec3 viewDir = fNormalize(viewPos - fFragPos);
        vec3 reflectDir = reflect(-lightDir, norm);
        vec3 halfwayDir = fNormalize(lightDir + viewDir);
        spec = pow(max(dot(norm, halfwayDir), 0.0), material.shininess);
    }
    float specular = material.specular * spec;

    // final
    vec3 result = (ambient + diffuse) * texture(material.texture, fTexCoords).rgb * light.color
