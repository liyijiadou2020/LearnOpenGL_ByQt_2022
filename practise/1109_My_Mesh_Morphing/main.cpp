...
#include <Qt3DExtras/qcuboidmesh.h> // 长方体
...
int main(int argc, char **argv)
{
    ...
    camera->setPosition(QVector3D(-10.0f, 20.0f, 20.0f));
    camera->setUpVector(QVector3D(0, 1, 0));
    camera->setViewCenter(QVector3D(0, 0, 0));

    // For camera controls 摄像机控制
    ...
    view.defaultFrameGraph()->setClearColor(Qt::darkGray);

    // Transform for mesh 环状网变化
    ...
    // create morh targets from geometry 变换模板
