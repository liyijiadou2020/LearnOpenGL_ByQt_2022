...
struct Material
{
    float ambient = 0.2; // 环境光，范围[0, 1]。如果为0表示环境中没有任何环境光，比如宇宙中
    float diffuse = 1; // 漫反射，范围[0, 1]。1表示只要接受到了光照就可以完全看得见。
    float specular = 0.2; // 镜面反射，范围[0,1]。0.2表示色子基本上不怎么有镜面反射。
    float shininess = 16; // 最终计算出来的镜面反射的效果。值越大，光斑越明显。16表示光斑不是那么明显。
};
