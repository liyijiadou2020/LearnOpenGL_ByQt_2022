...
void MyOpenGLWidget04::initializeGL()
{
    ...
    if (true) {
        ...
        auto _dice1 = new CubeModel();
        _dice1->setCamera(&m_camera);
        _dice1->setLight(&m_light);
        _dice1->setPos({ 0, 0, 0 });
        _dice1->init();
        models_house << _dice1;
        m_models << _dice1;

        auto _dice2 = new CubeModel();
        _dice2->setCamera(&m_camera);
        _dice2->setLight(&m_light);
        _dice2->setPos({ 0, 0, 0 });
        _dice2->init();
        models_house << _dice2;
        m_models << _dice2;
...
void MyOpenGLWidget04::setWireframe()
{
...
void MyOpenGLWidget04::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_L:     changeColorOfLight();                                 break;
    case Qt::Key_R:     m_light.setColor(QColor(255, 255, 255));              break;
    case Qt::Key_F:     setWireframe(); break;
    default: break;
    }
    update();
}
